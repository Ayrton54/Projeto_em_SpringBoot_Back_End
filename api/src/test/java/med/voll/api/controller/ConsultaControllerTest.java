...
import med.voll.api.domain.medico.EspecialidadeMedico;
...
class ConsultaControllerTest {
    ...
    void agendarCenario2() throws Exception {
        ...
        when(agendaConsulta.agendar(any())).thenReturn(dadosDetalhamentoConsulta);

        var response = mvc.perform(post("/consultas")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(dadosAgendamentoConsultaJson.write(
                                new DadosAgendamentoConsulta(1l, 1l,
                                        data, especialidade)
                        ).getJson())
                ).andReturn()
                .getResponse();
